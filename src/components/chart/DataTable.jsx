        ...
        <table className="w-full text-xs border-collapse">
          <thead className="sticky top-0 bg-card z-10">
            <tr className="border-b border-border">
              {columns.map(col => (
                <th key={col.name} className="px-3 py-2.5 text-left font-medium text-muted-foreground whitespace-nowrap border-r border-border/50 last:border-r-0">
                  <span>{col.name}</span>
          ...
          <tbody>
            {data.map((row, i) => (
              <tr key={i} className={`border-b border-border/40 hover:bg-muted/30 transition-colors ${i % 2 === 0 ? "" : "bg-muted/10"}`}>
                {columns.map(col => (
                  <td key={col.name} className="px-3 py-2 whitespace-nowrap text-foreground/80 border-r border-border/30 last:border-r-0">
                    {String(row[col.name] ?? "")}
                  </td>
                ))}
              </tr>
...
export default function DataTable({ data, columns }) {
  ...
  return (
    <>
      {fullscreen && <FullscreenTable data={data} columns={columns} onClose={() => setFullscreen(false)} />}
      ...
      <div className="space-y-2">
        <div className="flex items-center gap-2 w-full">
          ...
          >
            ...
            <span>Dataforhåndsvisning — {data.length} rader × {columns.length} kol.</span>
